...
def create_replacement_molecules(molecule, replacements):
    ...
    for src, dest in replacements:
        for match in re.finditer(src, molecule):
            mol = molecule[:match.start()] + dest + molecule[match.end():]
            mols.add(mol)
    return mols
...
# tranforming to outside of meeting the above patterns containing Rn, Y & Ar
# (as we are presuming that all other individual atoms are easy to transform
# to) then we can substiture them all for the same character to make things
