...
def parse_input(input_data):
    """
    Parses the input data to read out the starting configuration of crates and
    stacks.
    ...
    for i, line in enumerate(input_data):
        # Exit when we hit a blank line as that signifies the end of the
        # starting configuration
        if line == "":
            # Grab the remaining unparsed data as the list of move instructions
            moves_list = input_data[i+1:]
            break
        ...
        # Read along the line skipping 4 characters at a time so that we just
        # read the crate letters in each stack
        while char_index < len(line):
            # Read the current character. If it's an alphabetic character then
    ...
    return stacks, moves_list


def get_top_crates(stack_list):
    """
    Given a list of stacks creates a string representing the top crate from
    each stack and returns it
    """
    top_crates = ""
...
stacks1, moves = parse_input(data)
...
for move in moves:
    ...
    for _ in range(num_to_move):
        crate = stacks1[source_stack].pop()
        stacks1[dest_stack].append(crate)
